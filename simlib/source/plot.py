...
def make_sample_plot(fig, axs, U):
    ...
    axs[0,0].plot(U["t"], extract_param(U, "Body X Velocity"))
    axs[0,0].set_ylabel("$u_B$")
    axs[0,1].plot(U["t"], extract_param(U, "Body Z Velocity"))
    axs[0,1].set_ylabel("$w_B$")
    axs[1,0].plot(U["t"], extract_param(U, "Angular Velocity (deg/s)"))
    axs[1,0].set_ylabel("q")
    axs[1,1].plot(U["t"], extract_param(U, "Pitch Angle (deg)"))
    axs[1,1].set_ylabel(r'$\theta$')
    axs[2,0].plot(U["t"], extract_param(U, "Flight Path Angle (deg)"))
    axs[2,0].set_ylabel(r"$\gamma$")
    axs[2,1].plot(U["t"], extract_param(U, "Global z Pos"))
    axs[2,1].set_ylabel(r"$z_E$")
    axs[3,0].plot(U["t"], extract_param(U, "Angle of Attack (deg)"))
